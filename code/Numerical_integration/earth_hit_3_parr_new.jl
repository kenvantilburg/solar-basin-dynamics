...
@everywhere function hit_acc_file_tmin_integ_record(fname,dc,tmin,pos_end)
	open(fname,"r") do f
		tf = find_time(tmin,f,pos_end)
		if tf == nothing
			return []
		...
		a = read_full_csv_pos(f,tf[1],pos_end)
		#return hit_acc_integ_M(a,bz,dc)
		return hit_acc_integ_all_record(a,dc)
	end
end

@everywhere function hit_acc_file_tmin_integ_M_dcuts(fname,bz,dcuts,tmin,pos_end)
	open(fname,"r") do f
		tf = find_time(tmin,f,pos_end)
		if tf == nothing
			return nothing
		...
		a = read_full_csv_pos(f,tf[1],pos_end)
		#return hit_acc_integ_M(a,bz,dc)
		return hit_acc_integ_M_all_dcuts(a,bz,dcuts)
	end
end

@everywhere function hit_acc_file_tmin_integ_earthvel(fname,bz,dc,tmin,pos_end)
	open(fname,"r") do f
		tf = find_time(tmin,f,pos_end)
		if tf == nothing
			return nothing
		...
		a = read_full_csv_pos(f,tf[1],pos_end)
		#return hit_acc_integ_M(a,bz,dc)
		return hit_acc_integ_earthvel_all(a,bz,dc)
	end
end
...
@everywhere function bidxNew(bz,x0,x1)
	u -> clamp(floor(Int,bz*(u - x0)/(x1-x0)) + 1,1,bz)
end
...
@everywhere function hit_acc_integ_all_record(a,dc)
	nt = size(a)[2]
	t0  = a[1,1]
	dprev = 1.
	dmin = 1.
	latch = false
	hits = []
	vp_prev = SVector(0.,0.,0.)
	xe_prev = SVector(0.,0.,0.)
	ve_prev = SVector(0.,0.,0.)
	for i in 2:nt
		ap = a[:,i]
		dt = ap[1] - t0
		t0 = ap[1]
		function accfn(t,u,tp)
			xp = SVector(u[2,1,5],u[2,2,5],u[2,3,5])
			vp = SVector(u[1,1,5],u[1,2,5],u[1,3,5])
			xe = SVector(u[2,1,2],u[2,2,2],u[2,3,2])
			ve = SVector(u[1,1,2],u[1,2,2],u[1,3,2])
			d = norm(xp-xe)
			if d < dc
				...
				if d > dprev && latch
					#append!(hits,[[t,u[2,1,2],u[2,2,2],u[2,3,2],u[1,1,2],u[1,2,2],u[1,3,2],u[1,1,5],u[1,2,5],u[1,3,5]]])
					append!(hits,[[t,dprev,xe_prev[1],xe_prev[2],xe_prev[3],ve_prev[1],ve_prev[2],ve_prev[3],vp_prev[1],vp_prev[2],vp_prev[3]]])
					latch = false
				end
			end
			dprev = d
...
@everywhere function hit_acc_integ_M_all_dcuts(a,bz,dcuts)
	nt = size(a)[2]
	t0  = a[1,1]
	ifn = bidxNew(bz,0,2pi)
	acc = zeros(length(dcuts),bz)
	for i in 2:nt
		ap = a[:,i]
		dt = ap[1] - t0
		t0 = ap[1]
...
@everywhere function hit_acc_integ_earthvel_all(a,bz,dc)
	nt = size(a)[2]
	t0  = a[1,1]
	ifn = bidx(bz,-1.5,1.5)
	ifn1 = bidx(bz,0.,1.5)
	# So, doing accumulation differently here, then ...
	acc = zeros(bz,bz)
	for i in 2:nt
		ap = a[:,i]
		dt = ap[1] - t0
		...
		function accfn(t,u,tp)
			...
			d = norm(xp-xe)
			if d < dc
				vparr = dot(vp,ve) / norm(ve)
				vperp = sqrt(dot(vp,vp) - vparr^2)
				acc[ifn(vparr),ifn1(vperp)] += (t-tp)
			end
		end
		integ_diagnose(ap,dt,1e3*year,accfn)
	end
...
function run_script_2ee()
	...
	ld = length(dcuts)
	#saccs = SharedArray(zeros(bz,nf))
	saccs = SharedArray(zeros(ld,bz,nf))
	@sync @distributed for i in 1:nf
		if fnums[i] <= 256
			f = of[i]
			tmi = max(tmin,seps[1,1,i])
			if tmi < seps[1,2,i]
				hacc = hit_acc_file_tmin_integ_M_dcuts(f,bz,dcuts,tmi,pos_end[i])
				if hacc != nothing
					saccs[:,i] .= hacc
				end
			end
	...
	accs = sdata(saccs)
	@save "sun_acc_5_1e7_integ_M_all.jld2" tnow accs
end

function run_script_2eee()
	@load "snapshot_sun_5.jld2" of pos_end tnow seps fnums
	nf = length(of)
	tmin = (4.5e9 - 1e7)*year
	dc = 0.05
	red(a,b) = cat(a,b,dims=1)
	hits = @distributed red for i in 1:nf
		hacc = []
		if fnums[i] <= 256
			f = of[i]
			tmi = max(tmin,seps[1,1,i])
			if tmi < seps[1,2,i]
				hacc = hit_acc_file_tmin_integ_record(f,dc,tmi,pos_end[i])
			end
		end
		[(i,hacc)]
	end
	@save "sun_acc_5_1e7_integ_all_record.jld2" tnow hits
end
...
function run_script_4a()
	@load "snapshot_sun_5.jld2" of pos_end tnow seps fnums
	nf = length(of)
	tmin = (4.5e9 - 1e7)*year
	bz = 20
	dc = 0.02
	# Okay ... what do we want to accumulate into here?
	saccs = SharedArray(zeros(bz,bz,nf))
	@sync @distributed for i in 1:nf
		if fnums[i] <= 256
			f = of[i]
			tmi = max(tmin,seps[1,1,i])
			if tmi < seps[1,2,i]
				hacc = hit_acc_file_tmin_integ_earthvel(f,bz,dc,tmi,pos_end[i])
				if hacc != nothing
					saccs[:,:,i] .= hacc
				end
			end
	...
	accs = sdata(saccs)
	@save "sun_acc_5_1e7_integ_earthvel_all.jld2" tnow accs
end
