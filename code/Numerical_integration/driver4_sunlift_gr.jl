# used to test GR results.  Used to find that GR effects were more or less negligible for our purposes;
# just contributed to a slight change in the amount of time it took to lift a particle out of the sun.
...
const d0 = 0.0000400137

# incorporates GR effects
function pdot_gr!(dp,p,q,params,t)
	fill!(dp,0.)
	x3 = @SVector [norm(v3(q,i))^3 for i in 1:nplanets]
	for i in 1:nplanets
		xi = v3(q,i)
	...
	for i in 1:nplanets
		...
		# For the test particle, GR modifications ...
		if i == 5
			mfrac = xi1 < rsun ? 1 + d0 : 1 + d0*(rsun/xi1)
			pre *= mfrac
		end
		sv3(dp,i, -pre*xi/xi3)
		#@. dp[:,i] -= pre*xi/xi3
...
function integ_file(outfile,t0,tgap,tmax,tstep,print_first)
	...
	while true
		prob = DynamicalODEProblem(pdot_gr!,qdot!,v0p,x0p,tspan)
