# main file used for forwards and backward simulations
...
# solar potential approximated analytically (see SolarBasinBasic.jl)
...
# This function is responsible for actual integration of the system, and can pick up
# where a previous sim left off given a non-empty outfile when combined with extend_file
# below
...
function load_planets(i,earth=false)
	a = zeros(2 + npl*6)
...
function generate_files(i,outbase,seed,tmax,tgap,earth)
	while i < 1000
		...
		if earth
			t0 = load_planets(planets_i,earth=true)
			inject_around_earth()
			integ_file(outfile,t0,tgap,tmax,2e-4,true)
		else
			t0 = load_planets(planets_i)
